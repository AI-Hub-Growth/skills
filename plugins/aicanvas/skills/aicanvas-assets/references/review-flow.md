...
```bash
curl -s "$AICANVAS_HOST/api/ai/model-list?model_type=video"
```
...
```bash
curl -s "$AICANVAS_HOST/api/review-assets?model=seedance-2.0&page=1&page_size=100" \
...
```bash
curl -s -X POST "$AICANVAS_HOST/api/review-assets" \
  -H "Authorization: Bearer $AICANVAS_API_KEY" \
...
```bash
curl -s "$AICANVAS_HOST/api/review-assets/$ID" -H "Authorization: Bearer $AICANVAS_API_KEY"
```
...
```bash
curl -s "$AICANVAS_HOST/api/review-assets?model=seedance-2.0&type=image&page=1&page_size=100" \
  -H "Authorization: Bearer $AICANVAS_API_KEY"
```
