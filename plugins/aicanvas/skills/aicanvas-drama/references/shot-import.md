...
```bash
curl -s -X POST "$AICANVAS_HOST/api/shots/storyboards/$SB_ID/projects/$SP_ID/imports/preview" \
  -H "Authorization: Bearer $AICANVAS_API_KEY" \
...
```bash
curl -s -X POST "$AICANVAS_HOST/api/shots/storyboards/$SB_ID/projects/$SP_ID/imports/apply" \
  -H "Authorization: Bearer $AICANVAS_API_KEY" \
  -F 'file=@shots.md;type=text/markdown' \
