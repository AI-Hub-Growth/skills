...
每次进入任一 AICanvas skill，**第一步**都执行本检查；在检查完成前，不读取凭证、不查询模型、不恢复任务，也不调用业务接口。
...
- `10`：版本不一致，按下方对应宿主更新。
- 其他：无法确认版本。停止当前 AICanvas 工作并报告错误；不得把网络失败当作“已经最新”。
...
确认 `aicanvas@aicanvas` 显示的版本等于检查器报告的 `main` 版本。
