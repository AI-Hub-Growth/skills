#!/usr/bin/env python3
"""Compare the installed AICanvas plugin version with the main-branch manifest."""
...
LATEST_MANIFEST_URL = (
    "https://raw.githubusercontent.com/AI-Hub-Growth/skills/"
    "main/plugins/aicanvas/.codex-plugin/plugin.json"
)
...
def fetch_latest_version(url: str = LATEST_MANIFEST_URL, timeout: float = 10) -> str:
    request = urllib.request.Request(
        url,
        headers={"Accept": "application/json", "User-Agent": "aicanvas-skill-version-check"},
    )
    try:
...
def main() -> int:
    try:
        ...
        print(f"AICanvas skill 版本检查失败：{exc}", file=sys.stderr)
        return 2

    if current == latest:
        print(f"AICanvas skill 已是最新版本 {current}")
        return 0

    print(f"AICanvas skill 需要更新：本地 {current}，main {latest}")
    return 10
