...
```bash
curl -s -X POST "$LINC_HOST/api/upload/process-video" \
  -H "Authorization: Bearer $LINC_API_KEY" \
...
- **`referenceAssetIdList` 要 ID，不要 URL。** ID 从素材库接口拿（见 [linc-assets](../skills/linc-assets/SKILL.md)）。把 `POST /api/upload` 返回的 URL 塞进去必然失败。
...
`model-list` 里 `review_asset_enabled=true` 的模型，参考素材必须先过审核素材库（`POST /api/review-assets`），不能直接给 URL。见 [linc-assets](../skills/linc-assets/SKILL.md)。
