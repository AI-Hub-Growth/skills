...
用户在 codex / 编辑器里写好分镜表，导进灯虹出片。**不扣费。**

这是"在 agent 里写、在灯虹出片"的主路径。
...
```bash
curl -s "$LINC_HOST/api/shots/storyboards/$SB_ID/projects/$SP_ID/export?format=md" \
...
```bash
curl -s -X POST "$LINC_HOST/api/shots/storyboards/$SB_ID/projects/$SP_ID/imports/preview" \
  -H "Authorization: Bearer $LINC_API_KEY" \
...
```bash
curl -s -X POST "$LINC_HOST/api/shots/storyboards/$SB_ID/projects/$SP_ID/imports/apply" \
  -H "Authorization: Bearer $LINC_API_KEY" \
  -F 'file=@shots.md;type=text/markdown' \
