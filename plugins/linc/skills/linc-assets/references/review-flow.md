...
```bash
curl -s "$LINC_HOST/api/review-assets?model=seedance-2.0&page=1&page_size=100" \
...
```bash
curl -s -X POST "$LINC_HOST/api/upload" \
  -H "Authorization: Bearer $LINC_API_KEY" \
...
```bash
curl -s -X POST "$LINC_HOST/api/review-assets" \
  -H "Authorization: Bearer $LINC_API_KEY" \
...
```bash
curl -s "$LINC_HOST/api/review-assets/$ID" -H "Authorization: Bearer $LINC_API_KEY"
```
...
```bash
curl -s "$LINC_HOST/api/review-assets?model=seedance-2.0&type=image&page=1&page_size=100" \
  -H "Authorization: Bearer $LINC_API_KEY"
```
