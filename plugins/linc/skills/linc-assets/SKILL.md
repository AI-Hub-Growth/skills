...
name: linc-assets
description: |
  管理灯虹（LinC）素材库，以及视频生成前的合规预审核。不执行图片或视频生成；公开文档未给这些操作列出扣费错误码。
  触发场景："把这些素材整理到一个文件夹"、"新建个目录放角色图"、"素材库里找一下…"、
  "这几张图能过审吗"、"先把参考素材传上去"、"审核素材库满了帮我清一下"、"删掉这个文件夹"、
  "把这些文件移到…"、"传张图给我拿个 URL"。
  NOT for: 生成图片或视频 → linc-media。做短剧 → linc-drama。
  Chain signal: 素材传好/过审后要拿去生成 → linc-media 或 linc-drama。
---
...
```bash
# 某个文件夹里有什么
curl -s "$LINC_HOST/api/folders/contents?parent_id=root&page=1&page_size=40" \
...
```bash
curl -s -X POST "$LINC_HOST/api/folders" \
  -H "Authorization: Bearer $LINC_API_KEY" -H "Content-Type: application/json" \
...
```bash
curl -s "$LINC_HOST/api/folders/delete-impact?folder_id=folder-a&folder_id=folder-b" \
  -H "Authorization: Bearer $LINC_API_KEY"
```
...
```bash
curl -s -X DELETE "$LINC_HOST/api/folders" \
  -H "Authorization: Bearer $LINC_API_KEY" -H "Content-Type: application/json" \
...
```bash
curl -s "$LINC_HOST/api/quotas" -H "Authorization: Bearer $LINC_API_KEY"
```
...
```bash
curl -s -X DELETE "$LINC_HOST/api/review-assets/$ID" -H "Authorization: Bearer $LINC_API_KEY"
```
...
素材准备好了要生成 → [linc-media](../linc-media/SKILL.md)（单张图/单条视频）或 [linc-drama](../linc-drama/SKILL.md)（整部短剧）。
